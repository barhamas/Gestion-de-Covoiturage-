...
public class HistoristiqueReservationController extends DB implements Initializable {
    ...
    void supprimer(ActionEvent event){
        ...
        try{
            ...
            if("en cours...".equals(reservation.getEtat())){
                entityManager.remove(reservation);
                transaction.commit();
                viderChamps();
                loadTable();
            }else {
                Notification.NotifError("ERREUR", "Vous ne pouvez pas annuler les reservations passees");
            }
    ...
    void enregistrer(ActionEvent event) {
        try {
            ...
            if ( dateTfd.getValue() == null) {
                Notification.NotifError("ERREUR", "Veuillez choisir une date");
            }
            ...
            String numeroReservation= generateNumReservation(reservation.getTrajet().getTrajetId(),reservation.getVehicule().getVehiculeId(),annee,reservation.getUser().getUtilisateurId());
    ...
    void imprimer(ActionEvent event) {
        ...
        if (reservation != null) {
            try {
                ...
                Notification.NotifSuccess("SUCCESS", "Impression reussie !");
            } catch (Exception e) {
                e.printStackTrace();
                Notification.NotifError("ERREUR", "Echec de l'impression");
            }
        } else {
            Notification.NotifError("ERREUR", "Veuillez sélectionner une vente dans le tableau.");
        }
    }

    private void generateInvoice(Reservation reservation) throws Exception {

        String dest = DOWNLOADS_PATH + "Facture_" + reservation.getReservationId()  + ".pdf";

        PdfWriter writer = new PdfWriter(dest);
        PdfDocument pdf = new PdfDocument(writer);
        Document document = new Document(pdf);

        document.add(new Paragraph(" \t\t\t B' ArtCola Bradley\n\n").setFont(PdfFontFactory.createFont(StandardFonts.HELVETICA_BOLD)).setFontSize(28));
        document.add(new Paragraph("Facture de Vente").setFont(PdfFontFactory.createFont(StandardFonts.HELVETICA_BOLD)).setFontSize(20));
        document.add(new Paragraph("Numero de RESERVATION: "    + reservation.getNumeroReservation()));
        document.add(new Paragraph("Vehicule: "    + reservation.getVehicule().getMatricule()));
        document.add(new Paragraph("Trajet : "    + reservation.getTrajet().getNomTrajet()));
        document.add(new Paragraph("Date de RESERVATION: "    + reservation.getDate().toString()));
        document.add(new Paragraph("Tarif : "    + reservation.getPrix() + " Fcfa"));
    ...
    private String generateNumReservation(int id_banque, int id_client, int annee, int id_compte) {
        String ccc = String.valueOf(id_banque);
        String xxxx = String.valueOf(id_client);
        String yyyy = String.valueOf(annee);
        String zzzz = String.valueOf(id_compte);
        if (ccc.length() == 1) {
            ccc = "00" + ccc;
